...
fn menu(died: bool) {
    loop {
        ...
        let mut input = askquest("What would you like todo?: ", 0);

        //TODO maybe figure out some way to make numbers work with ease
        if input.trim() == "1" {
            input = "load".to_string();
        };
        if input.trim() == "2" {
            input = "new".to_string();
        };
        ...
        //TODO remove .trim()
        if input.trim() == "load" {
            println!("This does not work currently");
            continue;
        };
        if input.trim() == "new" {
            newgame();
        ...
        if input.trim() == "option" {
            println!("This does not work currently");
            continue;
        };
        if input.trim() == "help" {
            println!("uh don't die");
        }; //TODO make help bigger
        if input.trim() == "quit" {
            exit(1);
        };
        if input.trim() == "continue" {
            printt("um hold on", 250);
            continue;
        };
...
fn draw_map(stats: &mut Stats, world: &mut [u8; 262144]) {
    ...
    println!("\n\n");
    for x in 0..16 {
        print!("{}", id_index(world[cord_to_int(map_x - 8, map_y + 7 - x)]));
        print!("{}", id_index(world[cord_to_int(map_x - 8, map_y + 7 - x)]));
        print!("{}", id_index(world[cord_to_int(map_x - 7, map_y + 7 - x)]));
        print!("{}", id_index(world[cord_to_int(map_x - 7, map_y + 7 - x)]));
        print!("{}", id_index(world[cord_to_int(map_x - 6, map_y + 7 - x)]));
        print!("{}", id_index(world[cord_to_int(map_x - 6, map_y + 7 - x)]));
        print!("{}", id_index(world[cord_to_int(map_x - 5, map_y + 7 - x)]));
        print!("{}", id_index(world[cord_to_int(map_x - 5, map_y + 7 - x)]));

        print!("{}", id_index(world[cord_to_int(map_x - 4, map_y + 7 - x)]));
        print!("{}", id_index(world[cord_to_int(map_x - 4, map_y + 7 - x)]));
        print!("{}", id_index(world[cord_to_int(map_x - 3, map_y + 7 - x)]));
        print!("{}", id_index(world[cord_to_int(map_x - 3, map_y + 7 - x)]));
        print!("{}", id_index(world[cord_to_int(map_x - 2, map_y + 7 - x)]));
        print!("{}", id_index(world[cord_to_int(map_x - 2, map_y + 7 - x)]));
        print!("{}", id_index(world[cord_to_int(map_x - 1, map_y + 7 - x)]));
        print!("{}", id_index(world[cord_to_int(map_x - 1, map_y + 7 - x)]));

        print!("{}", id_index(world[cord_to_int(map_x, map_y + 7 - x)]));
        print!("{}", id_index(world[cord_to_int(map_x, map_y + 7 - x)]));
...
fn ent_id(world: [u8; 262144], x: u32, y: u32) -> u8 {
    return world[cord_to_int(x, y)];
}

fn update_map(mut world: &mut [u8; 262144], stats: &mut Stats) {
    // Update player
    for mut x in world.iter() {
        if x == &1u8 {
            x = &0u8;
        }
    }
...
fn player_input(input: &String, world: &mut [u8; 262144], stats: &mut Stats, inv: &mut [u32; 64]) {
    ...
    match words[0] {
        ...
        "move" => {
            ...
            let direction: u8 = match words[1] {
                "north" | "up" => 1,
                "east" | "right" => 2,
                "south" | "down" => 3,
                "west" | "left" => 4,
            ...
            if direction == 0 {
                println!(
                    "You need to have move formated like:\n\n{}\n",
                    "move DIRECTION #".red()
                );
                return;
            }
            ...
            if distance == 0 {
                ...
                while distance == 0 {
                    input = askquest("How far do you want to go?: ", 0);
        ...
        "kms" => exit(1),
        "help" => println!(
            "FQA:\n1. Why did my command not work?: You maybe entered it wrong or too many"
        ),
        _ => println!("sorry I didn't understand"),
    };
}
...
fn newgame() {
    ...
    loop {
        ...
        player_input(&input, &mut world, &mut stats, &mut inv);
    }
